...
def create_folium_map_with_images(pathgcp_before, pathgcp_after):
    ...
    for image_path in pathgcp_before.split(',') + pathgcp_after.split(','):
        try:
            ...
            if re.search('The GeoTIFF is invalid or is not cloud optimized:',
                         str(e)):
...
def run_example_generation(generate_examples_args,
    ...
    generated_examples_metric = DataflowMetricFetcher(
        job_project, job_name, 'generated_examples_count')
            ...
            if job_state == 'DONE':
                ...
                for k in range(20):
                    ...
                    tfrecord_path = os.path.join(
                        os.path.join(generate_examples_args['output_dir'],
                                     'examples'), file_directory)
...
def visualize_labeled_examples(path, max_examples=None):
    ...
    for record in tf.data.TFRecordDataset(path):
        ...
        if len(pre_images) < max_examples:
            pre_images.append(
                e.features.feature['pre_image_png'].bytes_list.value[0])
            post_images.append(
    ...
    ipyplot.plot_class_tabs(images,
                            labels,
...
def format_training_metrics(train_label_acc, train_label_auc, test_acc,
    ...
    html = """
        ...
        """.format(train_label_acc=train_label_acc,
            ...
            if get_logs_status == 0:
                with open(f'/tmp/{job_id}_log', 'r') as log_file:
                    ...
                    if job_id == train_job_id:
                        ...
                        curr_epoch = None
                        for log in log_data:
                            log_timestamp = timestamp_to_datetime(
                                log["timestamp"])
                            ...
                            if train_label_acc is None:
                                ...
                                if train_label_matches:
                                    train_label_epoch = min(
                                        (int(train_label_matches.groups()[0]) //
                                         total_num_img) + 1, total_num_epochs)
                                    train_label_epoch = f'{train_label_epoch}/{total_num_epochs}'
                                    train_label_auc = train_label_matches.groups(
                                    )[1]
                                    train_label_acc = train_label_matches.groups(
                                    )[2]
                        ...
                        if train_label_acc is not None and test_acc is not None:
                            if metrics_display is None:
                                ...
                                metrics_display.update(
                                    format_training_metrics(
                                        train_label_acc, train_label_auc,
                                        test_acc, test_auc, train_label_epoch,
                                        test_epoch,
                                        eval_most_recent_timestamp.strftime(
                                            "%Y-%m-%d, %H:%M:%S")))
...
def get_epoch_number(path_experiment, id_eval_job, checkpoint_selection,
    ...
    if checkpoint_selection == 'most_recent':
        ...
        os.system(
            f'gsutil cp {most_recent_epoch_file} /tmp/last_processed_epoch')
            ...
            with open(f'/tmp/{id_eval_job}_log', 'r') as log_file:
                ...
                for log in log_data:
                    ...
                    if test_matches:
                        ...
                        elif (int(test_matches.groups()[0]) > int(epoch_num_acc)
                              and float(test_matches.groups()[2])
                              == metrics_acc) or (float(
                        ...
                        elif (int(test_matches.groups()[0]) > int(epoch_num_auc)
                              and float(test_matches.groups()[1])
                              == metrics_auc) or (float(
...
def create_folium_map(geojson_path, pathgcp_before, pathgcp_after):
    ...
    style_function = lambda x: {
        ...
        'color':
            '#ff0000'
            if float(x['properties']['class_1']) >= 0.5 else '#00ff00',
        'fillColor':
            '#ff0000'
            if float(x['properties']['class_1']) >= 0.5 else '#00ff00',
        'fillOpacity':
