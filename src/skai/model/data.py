...
def decode_skai_record_bytes(
    record_bytes, image_size, use_post_disaster_only, load_small_images
):
...
def get_skai_dataloader_from_tfrecord(
    labeled_train_pattern: str,
    labeled_val_pattern: str,
    image_size: int,
    use_post_disaster_only: bool,
    load_small_images: bool,
):
...
class SkaiDatasetFactory:
  """Factory for Skai datasets."""

  @staticmethod
  def get_dataloader(
      source: str, config: ml_collections.ConfigDict
  ) -> Dataloader:
    """Create SKAI dataloader based on the source."""
    if source == ReadingRecordStratgey.USING_TFDS:
      ...
      return dataloader
    elif source == ReadingRecordStratgey.DIRECT_FROM_TFRECORD:
      return get_skai_dataloader_from_tfrecord(
