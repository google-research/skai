...
def get_model_config(
    ...
    image_variant: str,
    image_size: int,
    geofm_checkpoint_path: Optional[str],
) -> mlc.ConfigDict:
  ...
  Args:
    ...
    geofm_checkpoint_path: Path to the GeoFM checkpoint.
  ...
  if model_type == 'siglip':
    ...
    config = get_geofm_config(geofm_checkpoint_path)
...
def get_geofm_config(checkpoint_path: str) -> mlc.ConfigDict:
  """Returns the config for the GeoFM model.
  """
  if not checkpoint_path:
    raise ValueError('GeoFM checkpoint path cannot be empty.')
  config = mlc.ConfigDict()
  config.model = mlc.ConfigDict()
