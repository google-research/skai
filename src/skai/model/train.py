...
_USE_TFDS = flags.DEFINE_bool(
    'use_tfds',
    True,
    'If False the dataset will be read directly from tfrecord using tf.data'
    ' API. Otherwise, it will be read using tensoflow_datasets',
)
...
def main(_) -> None:
  ...
  if _USE_TFDS.value:
    ...
    dataloader = data.get_skai_dataloader_from_tfrecord(config)
