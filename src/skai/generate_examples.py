...
def read_labels_file(
    ...
    num_keep_labeled_examples: int) -> List[Tuple[float, float, float]]:
  ...
  Args:
    ...
    class_names: List of classes to be used as examples, e.g. ["undamaged",
      "damaged", "destroyed"].
    num_keep_labeled_examples: Number of labeled examples to keep
  ...
  if num_keep_labeled_examples:
    coordinates = coordinates[:num_keep_labeled_examples]
...
def get_dataflow_container_image(py_version: str) -> str:
  """Get default dataflow image based on Python version.
...
def get_gdal_env(gdal_env: list[str]) -> Dict[str, str]:
  """Parse list fo gdal_env to dictionary.

  Args:
    gdal_env: Environment configuration for GDAL

  Returns:
    Dictionay with variable as key and assigned value
  """
  gdal_env = {}
